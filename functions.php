...
<p><?php echo __( 'Descargas', 'appyn' ); ?>:<br>
    <input type="text" class="widefat" name="descargas" id="descargas" value="<?php echo get_datos_info( 'descargas' ); ?>">
</p>
...
?>
    ...
    jq1(function($) {
        ...
        $(".addLink").on('click', function() {
            $(".ElementLinks table tbody").append('<tr><td><input type="text" name="datos_download[' + count + '][link]" value="" class="widefat"></td><td><input type="text" name="datos_download[' + count + '][texto]" value="" class="widefat"></td><td><label><input type="checkbox" value="1" name="datos_download[' + count + '][follow]"> Follow</label></td><td><a href="javascript:void(0)" class="removeLink button">x</a></td></tr>');
            count++;
        });
...
function px_css_dark_theme() {
	...
	.section .bloque-app a, .section .bloque-app .px-postmeta, .section .bloque-app-second, .section a.more:hover,
   	#subheader.np #searchBox ul, .section .bloque-blog, .widget .widget-content ul li:hover a::before, .w75.bloque-imagen.bi_ll, .pxtd .entry.bx-info-install {
		background: #282d3a;
  	}		  
	...
	.aplication-single .bx-download, .bxt, .box .comments-title, .box #reply-title, #breadcrumbs a:hover, #main-site .error404 h1, #main-site .error404 h2, .aplication-single .data-app span b, .widget-title h2, .relacionados .bloque-app a:hover .title, .dl-verified, #comments ol.comment-list .trackback, #comments ol.comment-list .pingback, .widget a, .pxtd h3 {
		color: #FFF;
	}
...
function theme_scripts() {
	...
	wp_enqueue_script( 'px-js', get_bloginfo("template_directory").'/assets/js/js.min.js', array('jquery'), time(), true );
...
add_filter( 'body_class', function( $classes ) {
	...
	$aprpc = ( appyn_options( 'apps_per_row_pc' ) ) ? appyn_options( 'apps_per_row_pc' ) : 6;
	$add_class[] = 'aprp-'.$aprpc;
...
function single_info_title( $version = '' ) {
	...
	if( $filter_rvt ) { 
		$output = '<h1 class="box-title">'. str_replace( $version, '', $title ) . '</h1>';

		$output .= ( ! empty ( $version ) ? '<h4 class="version">'.$version.'</h4>' : '' );
	} else {
		$output = '<h1 class="box-title">'. $title . '</h1>';
	}
